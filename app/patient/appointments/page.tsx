...
export default function PatientAppointmentsPage() {
  ...
  const fetchAppointments = async () => {
    ...
    try {
      ...
      const specialtyIds =
        doctorProfiles?.map((dp: any) => dp.specialty_id).filter(Boolean) || [];
      ...
      if (specialtyIds.length > 0) {
        ...
        if (!specialtyError && specialties) {
          specialtiesMap = specialties.reduce(
      ...
      doctorProfiles?.forEach((dp: any) => {
        if (dp.user_id && dp.specialty_id) {
          userSpecialtyMap[dp.user_id] =
            specialtiesMap[dp.specialty_id] || "General Medicine";
        }
      });
      ...
      const combinedData = appointmentsData.map((appointment: any) => {
        ...
        return {
          ...
          doctor: {
            ...
            specialty:
              userSpecialtyMap[appointment.doctor_id] || "General Medicine",
          },
        };
      });
