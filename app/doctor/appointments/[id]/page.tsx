...
import { Calendar, Clock, User, Phone, Mail, MapPin, ArrowLeft, CheckCircle, XCircle, AlertCircle, Clock as ClockIcon } from 'lucide-react'
...
export default function DoctorAppointmentDetailPage() {
  ...
  const getAvailableActions = (status: string) => {
    ...
    if (status === 'pending_payment') {
      actions.push({ label: 'Confirm Appointment', value: 'confirmed', className: 'bg-green-600 hover:bg-green-700 text-white' })
    }
    if (status === 'confirmed') {
      actions.push({ label: 'Mark as Completed', value: 'completed', className: 'bg-blue-600 hover:bg-blue-700 text-white' })
    }
    if (status === 'pending_payment' || status === 'confirmed') {
      actions.push({ label: 'Cancel Appointment', value: 'cancelled', className: 'bg-red-600 hover:bg-red-700 text-white' })
    }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-400 text-sm">Loading appointment details...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/doctor/appointments" className="mt-4 inline-block text-blue-600 hover:underline">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              ...
              <span className="ml-3 text-sm text-gray-400 hidden sm:inline">Appointment Details</span>
            </div>
            <Link href="/doctor/appointments" className="text-sm text-gray-600 hover:text-gray-900 transition-colors">
              ← Back
            </Link>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-6 text-white">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center text-2xl text-white overflow-hidden">
                  {appointment.patient?.avatar_url ? (
                    <img src={appointment.patient.avatar_url} alt={appointment.patient.full_name} className="w-full h-full object-cover" />
                  ) : (
              ...
              <span className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium ${status.className}`}>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium text-gray-900 capitalize">
                  {appointment.consultation_type?.replace('_', ' ') || 'In-Person'}
                </p>
              </div>
              <div>
            ...
            <div className="border-t border-gray-200 pt-4">
              <h3 className="font-medium text-gray-900 mb-2">Patient Contact</h3>
