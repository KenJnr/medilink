...
export default function DoctorDashboard() {
  ...
  const fetchDoctorData = async () => {
    ...
    try {
      ...
      let activeProfile: DoctorProfile | null = null

      if (profileError) {
        if (profileError.code === 'PGRST116') {
          const { data: newProfile, error: createError } = await supabase
            .from('doctor_profiles')
            .insert({ user_id: user.id, consultation_fee: 0, approval_status: 'pending' })
            .select()
            .single()
      ...
      if (activeProfile?.approval_status === 'active') {
        ...
        if (!appointmentsError && appointmentsData) {
          ...
          const upcomingAppointments = appointmentsData.filter(
            (app) => (app.status === 'confirmed' || app.status === 'pending_payment') && new Date(app.starts_at) > now
          )
          const completedAppointments = appointmentsData.filter((app) => app.status === 'completed')
          const totalEarnings = completedAppointments.reduce((sum, app) => sum + (app.fee || 0), 0)
                ...
                {doctorProfile?.approval_status && (
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full ${
                      doctorProfile.approval_status === 'active' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                    }`}
                  >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">
            {isPending ? 'Complete your profile to get approved and start seeing patients.' : "Here's a summary of your practice."}
          </p>
        </div>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
          <div className="bg-linear-to-br from-blue-700 to-blue-500 relative rounded-xl px-4 py-3.5 shadow-sm overflow-hidden">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
          <Link href="/doctor/profile" className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md transition-shadow text-center">
            <Edit className="w-6 h-6 text-gray-600 mx-auto mb-2" />
            <p className="text-sm font-medium text-gray-700">Edit Profile</p>
          </Link>
          <Link href="/doctor/schedule" className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md transition-shadow text-center">
            <CalendarDays className="w-6 h-6 text-gray-600 mx-auto mb-2" />
            <p className="text-sm font-medium text-gray-700">Manage Schedule</p>
          </Link>
          <Link href="/doctor/appointments" className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md transition-shadow text-center">
        ...
        {!isPending && (
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              ...
              <Link href="/doctor/appointments" className="text-sm text-blue-600 hover:text-blue-800 transition-colors">
                View all →
              </Link>
            </div>
            <div className="p-4">
              {appointments.filter(
                (app) => (app.status === 'confirmed' || app.status === 'pending_payment') && new Date(app.starts_at) > new Date()
              ).length === 0 ? (
                ...
                <div className="space-y-3">
                  {appointments
                    .filter((app) => (app.status === 'confirmed' || app.status === 'pending_payment') && new Date(app.starts_at) > new Date())
                    .slice(0, 5)
                    .map((appointment) => {
                      ...
                      return (
                        ...
                        >
                          <div>
                            <p className="font-medium text-gray-900 text-sm">{appointment.users?.full_name || 'Unknown Patient'}</p>
                            <p className="text-xs text-gray-500">{appointment.reason || 'General Consultation'}</p>
                            <p className="text-xs text-gray-400 mt-1">
                              {new Date(appointment.starts_at).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })} ·{' '}
                              {new Date(appointment.starts_at).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                            </p>
                          </div>
                          <div className="flex items-center gap-3">
                            <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${status.className}`}>{status.label}</span>
                            <Link href={`/doctor/appointments/${appointment.id}`} className="text-sm text-blue-600 hover:text-blue-800 transition-colors">
                              View
                            </Link>
                          </div>
                        </div>
